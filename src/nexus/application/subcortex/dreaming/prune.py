...
class PruningUseCase:
    ...
    async def run(
        ...
        stale = await self._memory_repo.find_stale(
            access_threshold_days, limit, min_accesses, tenant_id=tenant_id
        )
        if stale:
            await self._memory_repo.delete_many([m.id for m in stale], tenant_id=tenant_id)
            result.vectors_pruned = len(stale)

        # --- 2. Synaptic decay: weaken everything unused ---
        for conn in await self._concept_repo.find_weakest(limit, tenant_id=tenant_id):
            if conn.decay(self._synapse.decay_rate, self._synapse.min_weight):
                await self._concept_repo.delete_connection(conn.id, tenant_id=tenant_id)
