...
class QdrantMemoryRepository(MemoryRepository):
    ...
    def __init__(self, host: str, port: int, embedder: EmbeddingProvider) -> None:
    ...
    async def retrieve(self, query: str, limit: int = 10, memory_types: Optional[list] = None) -> List[Memory]:
        ...
        return [self._from_payload(h.payload) for h in hits]

    async def get_by_id(self, memory_id: str) -> Optional[Memory]:
        points = await self._client.retrieve(collection_name=self.COLLECTION, ids=[memory_id])
        if not points:
            return None
        return self._from_payload(points[0].payload)
    ...
    async def find_stale(self, threshold_days: int, limit: int = 100) -> List[Memory]:
        ...
        filter_ = models.Filter(must=[models.FieldCondition(key="last_accessed_at", range=models.Range(lt=cutoff))])
        points = await self._client.scroll(
            collection_name=self.COLLECTION, limit=limit, filter_=filter_, with_payload=True
        )
        return [self._from_payload(p.payload) for p, _ in points]
    ...
    def _from_payload(self, p: dict) -> Memory:
        m = Memory(
            id=p.get("id", ""),
